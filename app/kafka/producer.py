...
class KafkaProducerClient():
  def __init__(self,logger,config:KafkaConfig):
    self._logger = logger
    self._config = config
    self._producer= KafkaProducer(bootstrap_servers=f'{self._config._host}:{self._config._port}'
                                  ,value_serializer= lambda m: avro_serializer(m,self._config._output_schema))
